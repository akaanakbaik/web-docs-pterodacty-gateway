...
function SearchBox({ query, setQuery }: { query: string; setQuery: (value: string) => void }) {
  return <label className="relative block min-w-0"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari: telegram, install, domain..." className="focus-ring h-11 w-full min-w-0 rounded-2xl border border-line bg-card/70 pl-9 pr-3 text-sm font-semibold text-ink outline-none transition placeholder:text-muted/70" /></label>;
}
