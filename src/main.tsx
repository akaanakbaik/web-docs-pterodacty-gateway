...
function App() {
  ...
  return (
    <div className="app-shell min-h-screen overflow-x-hidden bg-paper text-ink">
      ...
      <main className="relative mx-auto grid w-full max-w-7xl grid-cols-1 gap-4 px-3 pb-8 pt-20 sm:px-5 lg:grid-cols-[290px_minmax(0,1fr)] lg:gap-5 lg:px-6 lg:pb-12 lg:pt-24">
        <aside className="hidden min-w-0 lg:block"><Sidebar query={query} setQuery={handleSearch} activePath={route.path} filteredDocs={filteredDocs} /></aside>
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-line/80 bg-paper/82 backdrop-blur-xl">
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-3 sm:px-5 lg:px-6">
        <button className="min-w-0 flex items-center gap-2 text-left" onClick={() => navigateTo("/docs/overview")} aria-label="Akadev Pterodactyl Gateway Docs">
  ...
  return (
    <div className="sidebar-shell sticky top-24 max-h-[calc(100vh-7rem)] overflow-hidden rounded-[1.5rem] border border-line bg-card/78 p-3 shadow-soft backdrop-blur-xl">
      ...
      <div className="sidebar-scroll mt-3 max-h-[calc(100vh-13rem)] space-y-4 overflow-y-auto overscroll-contain pr-1 scrollbar-thin">
        {navGroups.filter((group) => group !== "Legal").map((group) => <div key={group}><p className="px-2 pb-1 text-[10px] font-extrabold uppercase tracking-[0.18em] text-muted">{group}</p><div className="space-y-1">{list.filter((doc) => doc.group === group).map((doc) => <DocNavButton key={doc.path} doc={doc} active={doc.path === props.activePath} />)}</div></div>)}
      </div>
    </div>
  );
}
...
function TerminalSimulation({ steps }: { steps: SimulationStep[] }) {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setActive((value) => (value + 1) % Math.max(steps.length, 1)), 2600);
    return () => window.clearInterval(timer);
  }, [steps.length]);
  const step = steps[active] ?? steps[0];
  if (!step) return null;
  return (
    <div className="mt-6 overflow-hidden rounded-[1.5rem] border border-line bg-[#151412] shadow-soft">
      <div className="flex items-center justify-between border-b border-white/10 px-3 py-2.5">
        <div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-red-400/80" /><span className="h-2.5 w-2.5 rounded-full bg-yellow-300/80" /><span className="h-2.5 w-2.5 rounded-full bg-green-400/80" /><span className="ml-2 text-[11px] font-extrabold uppercase tracking-[0.18em] text-white/45">terminal simulation</span></div>
        <button onClick={() => navigator.clipboard.writeText(step.terminal)} className="inline-flex items-center gap-1.5 rounded-full bg-white/8 px-2.5 py-1 text-[11px] font-bold text-white/80 transition hover:bg-white/14"><Copy className="h-3.5 w-3.5" /> Copy cmd</button>
      </div>
      <div className="p-4">
        <div className="mb-3 flex flex-wrap gap-1.5">{steps.map((item, index) => <button key={item.label} onClick={() => setActive(index)} className={`rounded-full px-2.5 py-1 text-[11px] font-bold transition ${index === active ? "bg-clay text-white" : "bg-white/8 text-white/55 hover:bg-white/12"}`}>{index + 1}. {item.label}</button>)}</div>
        <motion.div key={step.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="rounded-2xl border border-white/10 bg-white/[0.035] p-3">
          <p className="mb-2 text-xs font-extrabold text-white/70">{step.label}</p>
          <pre className="max-w-full overflow-x-auto whitespace-pre-wrap break-words text-[12px] leading-6 text-[#f2eadf] scrollbar-thin"><code><span className="text-clay">root@aka</span><span className="text-white/45">:</span><span className="text-sage">~/project</span><span className="text-white/45">$ </span>{step.terminal}<span className="terminal-cursor">▍</span></code></pre>
          <div className="my-3 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="terminal-progress h-full rounded-full bg-clay" /></div>
          <p className="rounded-xl bg-white/7 px-3 py-2 text-[12px] font-semibold leading-6 text-white/72">{step.result}</p>
        </motion.div>
      </div>
    </div>
  );
